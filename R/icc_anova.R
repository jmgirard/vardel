...
calc_aov_icc <- function(
  ...
  if (is.null(model_fit$result)) {
    ...
    v_a1 <- (a_1 * MSr + b_1 * MSe)^2 / ((a_1 * MSr)^2 / (k - 1) + (b_1 * MSe)^2 / ((n - 1) * (k - 1)))
    F_upper_a1 <- qf(1 - alpha/2, n - 1, v_a1)
    F_lower_a1 <- qf(alpha/2, n - 1, v_a1)
    icc_a1_lower <- n * (MSo - F_upper_a1 * MSe) / (F_upper_a1 * (k * MSr + (k * n - k - n) * MSe) + n * MSo)
    icc_a1_upper <- n * (MSo - F_lower_a1 * MSe) / (F_lower_a1 * (k * MSr + (k * n - k - n) * MSe) + n * MSo)
    # ICC(A,k) Approximation
    a_k <- iccs_ak / (n * (1 - iccs_ak))
    b_k <- 1 + (iccs_ak * (n - 1)) / (n * (1 - iccs_ak))
    v_ak <- (a_k * MSr + b_k * MSe)^2 / ((a_k * MSr)^2 / (k - 1) + (b_k * MSe)^2 / ((n - 1) * (k - 1)))
    F_upper_ak <- qf(1 - alpha/2, n - 1, v_ak)
    F_lower_ak <- qf(alpha/2, n - 1, v_ak)
    icc_ak_lower <- n * (MSo - F_upper_ak * MSe) / (F_upper_ak * (MSr - MSe) + n * MSo)
    icc_ak_upper <- n * (MSo - F_lower_ak * MSe) / (F_lower_ak * (MSr - MSe) + n * MSo)
